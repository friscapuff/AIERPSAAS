...
function ApprovalRuleFormModal({
  ...
  useEffect(() => {
    if (rule) {
      setFormData({
        ...
        targetApprovedStatus: rule.targetApprovedStatus,
        targetRejectedStatus: rule.targetRejectedStatus,
        isActive: rule.isActive,
        priority: rule.priority,
      });
      setConditions(rule.conditions?.map((c: any) => ({ field: c.field || '', operator: c.operator || 'EQ', value: String(c.value ?? '') })) || []);
      setLevels(rule.approvalLevels?.map((l: any) => ({
        level: l.level || 1,
        approver_role: l.approver_role || '',
        approver_user_id: l.approver_user_id || '',
        auto_approve_if: l.auto_approve_if || '',
      })) || []);
    } else {
      setFormData({ tableName: '', ruleName: '', description: '', triggerStatus: '', targetApprovedStatus: '', targetRejectedStatus: '', isActive: true, priority: 0 });
      setConditions([]);
  ...
  const updateLevel = (i: number, key: string, val: string) =>
    setLevels(levels.map((l, idx) => (idx === i ? { ...l, [key]: val } : l)));
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.targetApprovedStatus || !formData.targetRejectedStatus) {
      notify.error('Target approved and rejected statuses are required.');
      return;
    }
    if (levels.length === 0) {
      notify.error('At least one approval level is required.');
      return;
    }
    ...
    const payload = {
      ...
      target_approved_status: formData.targetApprovedStatus,
      targetApprovedStatus: formData.targetApprovedStatus,
      target_rejected_status: formData.targetRejectedStatus,
      targetRejectedStatus: formData.targetRejectedStatus,
      ...
      approval_levels: levels.filter((l) => l.approver_role || l.approver_user_id).map((l) => ({
        level: l.level,
        approver_role: l.approver_role || undefined,
        approver_user_id: l.approver_user_id || undefined,
        auto_approve_if: l.auto_approve_if || undefined,
      })),
      approvalLevels: levels.filter((l) => l.approver_role || l.approver_user_id).map((l) => ({
        level: l.level,
        approver_role: l.approver_role || undefined,
        approver_user_id: l.approver_user_id || undefined,
        auto_approve_if: l.auto_approve_if || undefined,
    ...
    try {
      if (rule) {
        ...
        notify.success('Approval rule updated.');
      } else {
      ...
      notify.error(err?.message || 'Failed to save approval rule.');
    }
  };
  ...
  return (
    <Modal
      ...
      description="Define when a document requires approval and who approves it"
      size="xl"
      footer={
        <>
      ...
      <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Input
              ...
              placeholder="PO over 1000 JOD approval"
              required
            />
            <Input
              ...
              hint="When record moves to this status, approval starts"
              required
            />
            <Input
              label="Priority"
            ...
            />
            <Input
              label="Target Approved Status"
              value={formData.targetApprovedStatus}
              onChange={(e) => setFormData({ ...formData, targetApprovedStatus: e.target.value })}
              placeholder="APPROVED"
              required
            />
            <Input
              label="Target Rejected Status"
              value={formData.targetRejectedStatus}
              onChange={(e) => setFormData({ ...formData, targetRejectedStatus: e.target.value })}
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h4 className="text-sm font-semibold text-surface-800">Conditions (optional)</h4>
            <Button variant="secondary" size="xs" leftIcon={<PlusIcon className="h-3.5 w-3.5" />} onClick={addCondition}>
              Add Condition
            </Button>
          </div>
          <p className="text-xs text-surface-400">If no conditions are set, the rule applies to ALL records reaching the trigger status.</p>
          {conditions.map((cond, i) => (
            <div key={i} className="grid grid-cols-[1fr_1fr_1fr_auto] gap-2 items-end p-2 bg-surface-50 rounded-lg border border-surface-200">
              ...
              <Input
                ...
                size="sm"
              />
              <button onClick={() => removeCondition(i)} className="p-1 text-surface-400 hover:text-danger-500 mb-0.5">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h4 className="text-sm font-semibold text-surface-800">Approval Levels</h4>
            <Button variant="secondary" size="xs" leftIcon={<PlusIcon className="h-3.5 w-3.5" />} onClick={addLevel}>
              Add Level
            </Button>
          </div>
          {levels.map((level, i) => (
            <div key={i} className="p-3 bg-surface-50 rounded-lg border border-surface-200 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-surface-600">Level {level.level}</span>
                {levels.length > 1 && (
                  <button onClick={() => removeLevel(i)} className="text-xs text-danger-500 hover:text-danger-700">Remove</button>
                )}
              </div>
              <div className="grid grid-cols-3 gap-2">
                <Input
                  ...
                  onChange={(e) => updateLevel(i, 'approver_role', e.target.value)}
                  placeholder="MANAGER"
                  size="sm"
                  hint="Role required to approve"
                />
                <Input
                  label="Specific User ID (optional)"
                  value={level.approver_user_id}
                  onChange={(e) => updateLevel(i, 'approver_user_id', e.target.value)}
                  placeholder="user-uuid"
                  size="sm"
                />
                <Input
                  label="Auto-Approve If (optional)"
                  value={level.auto_approve_if}
                  onChange={(e) => updateLevel(i, 'auto_approve_if', e.target.value)}
                  placeholder="amount < 500"
                  size="sm"
                  hint="Expression to skip this level"
                />
              </div>
            </div>
          ))}
    ...
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <Card padding="lg" className="text-center">
          <ShieldCheckIcon className="h-12 w-12 text-surface-300 mx-auto mb-3" />
          <h3 className="text-sm font-semibold text-surface-700">No approval rules defined</h3>
          <p className="text-xs text-surface-400 mt-1 mb-4">
            Define approval workflows so documents require sign-off before processing.
          </p>
            ...
            <Card key={rule.id} padding="md" className="group">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-surface-500">
                      Table: <span className="font-medium">{rule.tableName}</span>
                      {' · '}Trigger: <Badge size="sm" variant="warning">{rule.triggerStatus}</Badge>
                      {' → '}
                      <Badge size="sm" variant="success">{rule.targetApprovedStatus}</Badge>
                      {' / '}
                      <Badge size="sm" variant="default">{rule.targetRejectedStatus}</Badge>
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-surface-400">{rule.approvalLevels?.length || 0} level(s)</span>
                  <button
      ...
      <ConfirmModal
        ...
        message={`Delete "${deleteRule?.ruleName}"? Active approvals may be affected.`}
        confirmLabel="Delete"
